...
def get_inception_score(img_batch_arr, evaluator, splits=10):
    ...
    class_entrophy = entropy(np.bincount(np.argmax(preds, axis=1), minlength=10)/len(img_batch_arr))
    scores = []
    ...
    return mean_confidence, class_entrophy, np.mean(scores), np.std(scores)
