...
sns.set(context='paper', style='white', palette='Set2', font_scale=1, color_codes=False,
        rc={'font.family':'sans-serif', 'font.serif':['Palatino'], 'font.sans-serif':['DejaVu Sans'], 'text.usetex':True})
...
real_data = mnist_data[np.random.choice(range(len(mnist_data)), size=batch_size)]
...
for epoch in range(niter):
    ...
    while i < batches:
        ...
        if gen_iterations % 500 == 0:
            print('[%d/%d][%d/%d][%d] Loss_G:%f \t Loss_D:%f \t penalty_factor:%f \t D_real:%f \t D_fake:%f \t'
                  % (epoch, niter, i, batches, gen_iterations, errG, errD, penalty_fac, np.mean(errD_real),
                     np.mean(errD_fake)))
            fake = netG.predict(fixed_noise)
            mean_confidence, class_entrophy, score_mean, score_std = get_inception_score(fake, mnist_classifier)
        ...
        gen_iterations += 1
        score_mean, score_std = cifar10_evaluator(netG.predict(fixed_noise))
        inceps_score.append((score_mean, score_std))
        records.append([errG, errD, errD_real, errD_fake, penalty_fac])